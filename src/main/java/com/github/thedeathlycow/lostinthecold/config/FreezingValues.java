package com.github.thedeathlycow.lostinthecold.config;

public class FreezingValues {
    public static final boolean USE_HEART_OVERLAY = false;
    public static final int SECONDS_PER_FROST_RESIST = 30;
    public static final int BASE_FROST_RESISTANCE = 4;
    public static final int CHILLY_BIOME_FREEZE_RATE = 1;
    public static final int COLD_BIOME_FREEZE_RATE = 3;
    public static final int FREEZING_BIOME_FREEZE_RATE = 10;
    public static final int WARM_BIOME_FREEZE_RATE = -10;
    public static final int POWDER_SNOW_FREEZE_RATE = 100;
    public static final int ON_FIRE_FREEZE_RATE = -100;
    public static final float WET_FREEZE_RATE_MULTIPLIER = 1.5f;
}
