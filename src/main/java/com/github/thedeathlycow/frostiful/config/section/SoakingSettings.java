...
public class SoakingSettings {
    ...
    @AutoGen(category = CATEGORY, group = SOAKING_GROUP)
    @Translate.Name("Water potion soaking change multiplier")
    @SerialEntry(comment = "Multiplies the soaking change applied to entities struck by a water potion. The base soaking change will be 50% of the affected player's maximum wet ticks. Must be at least 0.")
    @FloatField(min = 0, format = "%.2f")
    float waterPotionSoakingChangeMultiplier = 1.0f;
    ...
    public int soakingFromWaterPotion(int maxSoakingTicks) {
        return Mth.floor(0.5f * maxSoakingTicks * this.waterPotionSoakingChangeMultiplier);
    }

    @AutoGen(category = CATEGORY, group = DRYING_GROUP)
    @Translate.Name("Enable light drying")
    @SerialEntry(comment = "When enabled, entities will be able to be dried from being the prescence of block light. The strength of the drying increases linearly with the block light level.")
    @TickBox
