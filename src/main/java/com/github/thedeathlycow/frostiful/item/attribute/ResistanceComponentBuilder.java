...
public final class ResistanceComponentBuilder {
    ...
    public static void applyLegacyArmorMaterialTags(ItemStack stack) {
        if (!stack.has(FDataComponentTypes.FROST_RESISTANCE) && stack.getItem() instanceof ArmorItem armor) {
            Holder<ArmorMaterial> material = armor.getMaterial();
            if (material.is(ArmorMaterialTags.VERY_RESISTANT_TO_COLD)) {
                stack.set(FDataComponentTypes.FROST_RESISTANCE, FrostResistanceComponent.VERY_PROTECTIVE);
            } else if (material.is(ArmorMaterialTags.RESISTANT_TO_COLD)) {
                stack.set(FDataComponentTypes.FROST_RESISTANCE, FrostResistanceComponent.PROTECTIVE);
            } else if (material.is(ArmorMaterialTags.VERY_WEAK_TO_COLD)) {
                stack.set(FDataComponentTypes.FROST_RESISTANCE, FrostResistanceComponent.VERY_HARMFUL);
            } else if (material.is(ArmorMaterialTags.WEAK_TO_COLD)) {
    ...
    private static ItemAttributeModifiers getOrDefault(DataComponentMap.Builder builder, Item item) {
        ...
        if (component.modifiers().isEmpty()) {
            component = item.getDefaultAttributeModifiers();
        }
